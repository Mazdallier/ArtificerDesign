...
public class ADBlock extends Block {
	 ...
	 * 
	 * @param material
	 *            The material of said block
	 * @param name
	 *            This is the UnlocalizedName
	 */
	...
	public static final Block RedGemOre = new RedGemOre(Names.RED_GEM,
			BlockTexture.RedGemOre);
	...
	public static final Block DwarvenStone = new DwarvenStone(Names.DSTONE,
			BlockTexture.DwarvenStone);
	public static final Block GreenFire = new FakeFire()
			.setBlockName(Names.GFIRE);
	public static final Block DVine = new driedVine(Names.DVINE,
			BlockTexture.DriedVine);
	...
	/**
	 * checks if it is not touching said side on y axis: args world, x, y, z,
	 * blockID, side
	 */
	public static boolean blockIsSide(World world, int x, int y, int z,
			Block block, int side) {
	...
	/**
	 * places a block relative to the coords and side: args world, x, y, z, side
	 * of block
	 */
	...
	/**
	 * fills an area with said blockID args:
	 * world,minX,minY,inZ,maxX,maxY,maxZ,BlockId,checks if it can replace
	 * already existing blocks (true == will not override)
	 **/
	public static void fill(World world, int mx, int my, int mz, int mX,
			int mY, int mZ, Block block, int meta, boolean check) {
		int x, y, z, X, Y, Z;
				...
				for (int fz = z; fz <= Z; fz++) {
					if (check) {
						if (world.getBlock(fx, fy, fz) == Blocks.air) {
							...
							world.setBlockMetadataWithNotify(fx, fy, fz, meta,
									3);
						} else
