...
public class SkillBookGui extends GuiScreen{
	...
	public SkillBookGui(EntityPlayer player){
		...
		if(temp != null) race = this.getRacefromBook(temp);
	}

	protected void mouseClicked(int mouseX,int mouseY,int clicked){
		if(clicked == 0){
			if(this.isMain) currentTab = getClickedTab(x,y,mouseX,mouseY);
	...
	public void drawScreen(int mouseX,int mouseY,float par3){
		...
		if(this.isMain){
			...
			if(this.currentPage > 0) if(getNewPage(mouseX,mouseY) == -1)
				this.drawTexturedModalRect(arrowX,arrowY,0,textHeight + arrowGap,arrowW,arrowH);
			else this.drawTexturedModalRect(arrowX,arrowY,0,textHeight + 1,arrowW,arrowH);

			if(hasNextPage()) if(getNewPage(mouseX,mouseY) == 1)
				this.drawTexturedModalRect(arrowX + arrowSpace,arrowY,17,textHeight + arrowGap,arrowW,arrowH);
			else this.drawTexturedModalRect(arrowX + arrowSpace,arrowY,17,textHeight + 1,arrowW,arrowH);

			this.font.drawString(this.currentPage + 1 + "/" + (this.maxPages + 1),this.arrowX + (this.arrowSpace / 2),this.arrowY,Color.BLACK.getRGB());

			if(page != null) for(int i = 0; i < this.page.size(); i++){
	...
	Tab getClickedTab(int x,int y,int mouseX,int mouseY){
		for(int i = 0; i < Tab.values().length; i++){
			...
			if(minX <= mouseX && maxX >= mouseX && minY <= mouseY && maxY >= mouseY) return Tab.values()[i];
		}
	...
	boolean renderPage(int mouseX,int mouseY,boolean clicked){
		...
		if(clicked) pageNumber += this.getNewPage(mouseX,mouseY);
	...
	int getNewPage(int mouseX,int mouseY){
		...
		if(minX <= mouseX && maxX >= mouseX && minY <= mouseY && maxY >= mouseY) return -1;
	...
	PlayerRace getRacefromBook(ItemStack item){
		if(item.getItem() == ADItems.HumanBook) return PlayerRace.HUMAN;
		if(item.getItem() == ADItems.DwarfBook) return PlayerRace.DWARF;
		if(item.getItem() == ADItems.ElfBook) return PlayerRace.ELF;
		if(item.getItem() == ADItems.OrcBook) return PlayerRace.ORC;
