...
public class VineMat extends ADItem{
	...
	private void init(World world,int x,int y,int z,ItemStack item){
		...
		if(Math.abs(mX - mx) <= getRange(item) && Math.abs(mZ - mz) <= getRange(item)) set(world,mx,mX,y,mz,mZ,item);
	}
	...
	public IIcon getIcon(ItemStack item,int pass){
		...
		if(pass == 0) if(damage < 3) return IconArray[0];
		else if(damage < 6) return IconArray[1];
		else return IconArray[2];
