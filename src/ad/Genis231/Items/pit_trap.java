...
public class pit_trap extends Item {
	...
	public pit_trap(int id) {
		...
		setTextureName(Ref.Resource_FOLDER + "PitTrap_ST1");
	}
	...
	public boolean onItemUse(ItemStack itemStack, EntityPlayer player,
			World world, int x, int y, int z, int side, float Bx, float By,
			float Bz) {
		switch (side) {
		case 2:// z-1
	...
	void range(World world, EntityPlayer player, int x, int y, int z) {
		...
		if (Math.abs(dimX) <= max && Math.abs(dimZ) <= max) {
			...
			if (CheckArea(world, mx, y, mz, mX, mZ)
					&& CheckBorder(world, mx, y, mz, mX, mZ))
				blocks.fill(world, mx, y, mz, mX, y, mZ, blocks.FPTrap.blockID,
						true);
			PTBlock.toggle = true;
		}
	}

	public static boolean CheckArea(World world, int mx, int y, int mz, int mX,
			int mZ) {
		int x, z, X, Z, i = 0;
		...
		for (int q = x; q <= X; q++) {
			for (int w = z; w <= Z; w++) {
				if (world.getBlockId(q, y - 1, w) != 0
						|| world.getBlockId(q, y + 1, w) != 0)
					i++;
				if (world.getBlockId(q, y, w) != 0
						&& world.getBlockId(q, y, w) != blocks.FPTrap.blockID)
					i++;
			}
		}
		if (i != 0)
			System.out.println("CheckArea: " + i);
		return i == 0;
	}

	public static boolean CheckBorder(World world, int mx, int y, int mz,
			int mX, int mZ) {
		int x, z, X, Z, i = 0;
		...
		if (i != 0)
			System.out.println("CheckBorder: " + i);
		return i == 0;
	}
}
